...
$$
\Omega^\alpha = \llbracket -\alpha\_\text{offset}-\tilde{i}_0, -\alpha\_\text{offset}-\tilde{i}_0 + \text{length}(\tilde{\alpha}) -1 \rrbracket
$$

With $\alpha\_\text{offset}=0$ you are in the "usual situation":

[[file:a_offset.png]]

and this does not depend on $\tilde{i}_0$ (filter definition does not depend on the language!).
...
function direct_conv!{T}(tilde_α::StridedVector{T},
    ...
    direct_conv!(tilde_α,
                 UnitRange(-λ*α_offset,length(tilde_α)-λ*α_offset-1),
                 λ,
...
# a_offset.png http://pixorblog.files.wordpress.com/2016/07/a_offset2.png
