...
and
$$
k+\lambda i = k+ \lambda \tilde{i} + \lambda (l(\Omega^\alpha) - \tilde{i}_0)
$$
...
\lambda \tilde{i} + \underbrace{\lambda (l(\Omega^\alpha) -
\tilde{i}_0)}_{\beta\_\text{offset}}]} $$
...
#+begin_src julia :name jl_set.jl
...
#+begin_src julia :name jl_boundary
...
function boundaryExtension_mirror{T}(β::StridedVector{T},
    ...
    kmax-abs(kmax-kmin-mod(k-kmin,2*(kmax-kmin)))
end
